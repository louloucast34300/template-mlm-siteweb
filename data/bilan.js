export const bilan = [
    {   
        id_forma:32,
        title:'Bilan de compétences',
        sub_title: '',
        nb_hours : 10,
        price:'1.000€',
        type: 'BILAN DE COMPETENCE',
        level:'Débutant',
        img:'../images/formations/bilan1.png',
        page_detail:'/formationDetail.html',
        page_CPF:'#',
        info:{
            ref:'ANGLAISDEB2_APP',
            version:'28/04/2022',
            public_target:'Tout public actif en emploi ou en recherche d’emploi, tous secteurs confondus et ce quel que soit le niveau d’étude ou de statut.',//
            level_entry:'pas de prérequis',
            xp_pro:'pas de prérequis',
            langue:'pas de prérequis',
            stuff:'disposer d’un ordinateur et d’une connexion internet.',
            financement:'par le CPF',
            rythme:'À définir avec le bénéficiaire',
            accompagnement:`Dans le cadre de notre prestation, vous êtes accompagné par une équipe composée d'un référent pédagogique et administratif, d'un référent technique et d'un référent handicap.`,
            pedagogie : `<br/>Le bilan de compétences s’appuie sur une alternance de rendez-vous individuels en visioconférence et de travaux guidés sur différents supports réflexifs :<br/>
            - Des tests de personnalité, d’intérêts et de motivation sur l’outil Pass’Avenir<br/>
            - Des tests d’évaluation des compétences transversales avec l’outil Transférence<br/>
            - Une base documentaire riche de +1200 fiches métiers contenant des informations sur le tissu professionnel territorial et les évolutions du marché`,
            evaluation:{
                avant:'évaluation des besoins lors d’un entretien en phase préliminaire',
                pendant:'Sur Pass-Avenir : évaluation de la personnalité, des intérêts, du profil professionnel, des aptitudes et des conditions de travail souhaitées. Sur Transférence : évaluation des compétences transversales',
                fin:'questionnaire de satisfaction',
                apres:'questionnaire de suivi à 6 mois',
            },
            certificat:'Certiﬁcation en anglais LanguageCert Test of English LTE (écouter, lire) – niveaux A1-C2 (RS5453).',
            validation_partielle:'non',
            equivalence:'aucune',
            level_end:'sans niveau spécifique'
        },
        content:{
           nb_module:0,
           modules:[
                `Entretien d'accueil<br/>
                Ce premier entretien gratuit nous permettra d'analyser votre demande et d'identifier vos besoins afin de vous proposer le parcours le plus adapté.`,
                `Phase préliminaire <br/>
                On fait le point sur les aspects réglementaires du bilan de compétences`,
                "On analyse ensemble vos attentes pour définir au mieux les objectifs de votre bilan de compétences", 
                "On aborde votre parcours professionnel et personnel afin d'adapter les outils à votre projet et les conditions de déroulement",
                "On vous accompagne dans la création de votre compte Parcouréo auquel vous aurez accès 24h/24",
                `Phase d'investigation <br/>
                On identifie vos compétences transversales et vos motivations à l'aide de Pass'Avenir et Transférence pour définir votre profil professionnel`,
                "On met en perspective votre profil et votre projet afin d'analyser les solutions et les alternatives envisageables",
                "On affine les possibilités au regard du marché du travail et de la formation",
                `Phase de conclusion <br/>
                On vous accompagne dans l'appropriation des résultats`,
                "On codéfinit les actions à suivre pour que vous puissiez réaliser votre projet d'évolution professionnelle",
                "On vous remet une synthèse qui reprend tout le travail effectué durant votre bilan de compétences",
                "On se contacte 6 mois plus tard pour faire le point sur les actions réalisées"],
            resume : []
        },
        competence:{
            title:[
               ""
            ],
            resume: [
                "Analyser ses compétences professionnelles et personnelles, ses aptitudes et ses motivations valider ou définir un projet professionnel voire un projet de formation valoriser ses compétences afin d'enrichir son profil professionnel"
            ],
        }
    },
    {   
        id_forma:33,
        title:'Bilan de compétences',
        sub_title: '',
        nb_hours : 10,
        price:'1.800€',
        type: 'BILAN DE COMPETENCE',
        level:'Débutant',
        img:'../images/formations/bilan1.png',
        page_detail:'/formationDetail.html',
        page_CPF:'#',
        info:{
            ref:'ANGLAISDEB2_APP',
            version:'28/04/2022',
            public_target:'Tout public actif en emploi ou en recherche d’emploi, tous secteurs confondus et ce quel que soit le niveau d’étude ou de statut.',//
            level_entry:'pas de prérequis',
            xp_pro:'pas de prérequis',
            langue:'pas de prérequis',
            stuff:'disposer d’un ordinateur et d’une connexion internet.',
            financement:'par le CPF',
            rythme:'À définir avec le bénéficiaire',
            accompagnement:`Dans le cadre de notre prestation, vous êtes accompagné par une équipe composée d'un référent pédagogique et administratif, d'un référent technique et d'un référent handicap.`,
            pedagogie : `<br/>Le bilan de compétences s’appuie sur une alternance de rendez-vous individuels en visioconférence et de travaux guidés sur différents supports réflexifs :<br/>
            - Des tests de personnalité, d’intérêts et de motivation sur l’outil Pass’Avenir<br/>
            - Des tests d’évaluation des compétences transversales avec l’outil Transférence<br/>
            - Une base documentaire riche de +1200 fiches métiers contenant des informations sur le tissu professionnel territorial et les évolutions du marché`,
            evaluation:'L’évaluation des compétences en cours de formation s’effectue par QCM et lors des temps d’échanges en anglais avec le formateur <br> La certiﬁcation des compétences se déroule lors d’un test adaptatif d’une heure réalisé à distance avec une surveillance en ligne.',
            certificat:'Certiﬁcation en anglais LanguageCert Test of English LTE (écouter, lire) – niveaux A1-C2 (RS5453).',
            validation_partielle:'non',
            equivalence:'aucune',
            level_end:'sans niveau spécifique'
        },
        content:{
           nb_module:0,
           modules:[
                `Entretien d'accueil<br/>
                Ce premier entretien gratuit nous permettra d'analyser votre demande et d'identifier vos besoins afin de vous proposer le parcours le plus adapté.`,
                `Phase préliminaire <br/>
                On fait le point sur les aspects réglementaires du bilan de compétences`,
                "On analyse ensemble vos attentes pour définir au mieux les objectifs de votre bilan de compétences", 
                "On aborde votre parcours professionnel et personnel afin d'adapter les outils à votre projet et les conditions de déroulement",
                "On vous accompagne dans la création de votre compte Parcouréo auquel vous aurez accès 24h/24",
                `Phase d'investigation <br/>
                On identifie vos compétences transversales et vos motivations à l'aide de Pass'Avenir et Transférence pour définir votre profil professionnel`,
                "On met en perspective votre profil et votre projet afin d'analyser les solutions et les alternatives envisageables",
                "On affine les possibilités au regard du marché du travail et de la formation",
                `Phase de conclusion <br/>
                On vous accompagne dans l'appropriation des résultats`,
                "On codéfinit les actions à suivre pour que vous puissiez réaliser votre projet d'évolution professionnelle",
                "On vous remet une synthèse qui reprend tout le travail effectué durant votre bilan de compétences",
                "On se contacte 6 mois plus tard pour faire le point sur les actions réalisées"],
            resume : []
        },
        competence:{
            title:[
               ""
            ],
            resume: [
                "Analyser ses compétences professionnelles et personnelles, ses aptitudes et ses motivations valider ou définir un projet professionnel voire un projet de formation valoriser ses compétences afin d'enrichir son profil professionnel"
            ],
        }
    },
    {   
        id_forma:34,
        title:`Bilan de compétences avec accompagnement vers l'action`,
        sub_title: '',
        nb_hours : 15,
        price:'2.700€',
        type: 'BILAN DE COMPETENCE',
        level:'Débutant',
        img:'../images/formations/bilan1.png',
        page_detail:'/formationDetail.html',
        page_CPF:'#',
        info:{
            ref:'ANGLAISDEB2_APP',
            version:'28/04/2022',
            public_target:'Tout public actif en emploi ou en recherche d’emploi, tous secteurs confondus et ce quel que soit le niveau d’étude ou de statut.',//
            level_entry:'pas de prérequis',
            xp_pro:'pas de prérequis',
            langue:'pas de prérequis',
            stuff:'disposer d’un ordinateur et d’une connexion internet.',
            financement:'par le CPF',
            rythme:'À définir avec le bénéficiaire',
            accompagnement:`Dans le cadre de notre prestation, vous êtes accompagné par une équipe composée d'un référent pédagogique et administratif, d'un référent technique et d'un référent handicap.`,
            pedagogie : `<br/>Le bilan de compétences s’appuie sur une alternance de rendez-vous individuels en visioconférence et de travaux guidés sur différents supports réflexifs :<br/>
            - Des tests de personnalité, d’intérêts et de motivation sur l’outil Pass’Avenir<br/>
            - Des tests d’évaluation des compétences transversales avec l’outil Transférence<br/>
            - Une base documentaire riche de +1200 fiches métiers contenant des informations sur le tissu professionnel territorial et les évolutions du marché`,
            evaluation:'L’évaluation des compétences en cours de formation s’effectue par QCM et lors des temps d’échanges en anglais avec le formateur <br> La certiﬁcation des compétences se déroule lors d’un test adaptatif d’une heure réalisé à distance avec une surveillance en ligne.',
            certificat:'Certiﬁcation en anglais LanguageCert Test of English LTE (écouter, lire) – niveaux A1-C2 (RS5453).',
            validation_partielle:'non',
            equivalence:'aucune',
            level_end:'sans niveau spécifique'
        },
        content:{
           nb_module:0,
           modules:[
                `Entretien d'accueil<br/>
                Ce premier entretien gratuit nous permettra d'analyser votre demande et d'identifier vos besoins afin de vous proposer le parcours le plus adapté.`,
                `Phase préliminaire <br/>
                On fait le point sur les aspects réglementaires du bilan de compétences`,
                "On analyse ensemble vos attentes pour définir au mieux les objectifs de votre bilan de compétences", 
                "On aborde votre parcours professionnel et personnel afin d'adapter les outils à votre projet et les conditions de déroulement",
                "On vous accompagne dans la création de votre compte Parcouréo auquel vous aurez accès 24h/24",
                `Phase d'investigation <br/>
                On identifie vos compétences transversales et vos motivations à l'aide de Pass'Avenir et Transférence pour définir votre profil professionnel`,
                "On met en perspective votre profil et votre projet afin d'analyser les solutions et les alternatives envisageables",
                "On affine les possibilités au regard du marché du travail et de la formation",
                `Phase de conclusion <br/>
                On vous accompagne dans l'appropriation des résultats`,
                "On codéfinit les actions à suivre pour que vous puissiez réaliser votre projet d'évolution professionnelle",
                "On vous remet une synthèse qui reprend tout le travail effectué durant votre bilan de compétences",
                "On se contacte 6 mois plus tard pour faire le point sur les actions réalisées"],
            resume : []
        },
        competence:{
            title:[
               ""
            ],
            resume: [
                "Analyser ses compétences professionnelles et personnelles, ses aptitudes et ses motivations valider ou définir un projet professionnel voire un projet de formation valoriser ses compétences afin d'enrichir son profil professionnel"
            ],
        }
    },
    {   
        id_forma:35,
        title:`Bilan de compétences avec accompagnement renforcé vers l'action`,
        sub_title: '',
        nb_hours : 20,
        price:'3.500€',
        type: 'BILAN DE COMPETENCE',
        level:'Débutant',
        img:'../images/formations/bilan1.png',
        page_detail:'/formationDetail.html',
        page_CPF:'#',
        info:{
            ref:'ANGLAISDEB2_APP',
            version:'28/04/2022',
            public_target:'Tout public actif en emploi ou en recherche d’emploi, tous secteurs confondus et ce quel que soit le niveau d’étude ou de statut.',//
            level_entry:'pas de prérequis',
            xp_pro:'pas de prérequis',
            langue:'pas de prérequis',
            stuff:'disposer d’un ordinateur et d’une connexion internet.',
            financement:'par le CPF',
            rythme:'À définir avec le bénéficiaire',
            accompagnement:`Dans le cadre de notre prestation, vous êtes accompagné par une équipe composée d'un référent pédagogique et administratif, d'un référent technique et d'un référent handicap.`,
            pedagogie : `<br/>Le bilan de compétences s’appuie sur une alternance de rendez-vous individuels en visioconférence et de travaux guidés sur différents supports réflexifs :<br/>
            - Des tests de personnalité, d’intérêts et de motivation sur l’outil Pass’Avenir<br/>
            - Des tests d’évaluation des compétences transversales avec l’outil Transférence<br/>
            - Une base documentaire riche de +1200 fiches métiers contenant des informations sur le tissu professionnel territorial et les évolutions du marché`,
            evaluation:'L’évaluation des compétences en cours de formation s’effectue par QCM et lors des temps d’échanges en anglais avec le formateur <br> La certiﬁcation des compétences se déroule lors d’un test adaptatif d’une heure réalisé à distance avec une surveillance en ligne.',
            certificat:'Certiﬁcation en anglais LanguageCert Test of English LTE (écouter, lire) – niveaux A1-C2 (RS5453).',
            validation_partielle:'non',
            equivalence:'aucune',
            level_end:'sans niveau spécifique'
        },
        content:{
           nb_module:0,
           modules:[
                `Entretien d'accueil<br/>
                Ce premier entretien gratuit nous permettra d'analyser votre demande et d'identifier vos besoins afin de vous proposer le parcours le plus adapté.`,
                `Phase préliminaire <br/>
                On fait le point sur les aspects réglementaires du bilan de compétences`,
                "On analyse ensemble vos attentes pour définir au mieux les objectifs de votre bilan de compétences", 
                "On aborde votre parcours professionnel et personnel afin d'adapter les outils à votre projet et les conditions de déroulement",
                "On vous accompagne dans la création de votre compte Parcouréo auquel vous aurez accès 24h/24",
                `Phase d'investigation <br/>
                On identifie vos compétences transversales et vos motivations à l'aide de Pass'Avenir et Transférence pour définir votre profil professionnel`,
                "On met en perspective votre profil et votre projet afin d'analyser les solutions et les alternatives envisageables",
                "On affine les possibilités au regard du marché du travail et de la formation",
                `Phase de conclusion <br/>
                On vous accompagne dans l'appropriation des résultats`,
                "On codéfinit les actions à suivre pour que vous puissiez réaliser votre projet d'évolution professionnelle",
                "On vous remet une synthèse qui reprend tout le travail effectué durant votre bilan de compétences",
                "On se contacte 6 mois plus tard pour faire le point sur les actions réalisées"],
            resume : []
        },
        competence:{
            title:[
               ""
            ],
            resume: [
                "Analyser ses compétences professionnelles et personnelles, ses aptitudes et ses motivations valider ou définir un projet professionnel voire un projet de formation valoriser ses compétences afin d'enrichir son profil professionnel"
            ],
        }
    },
]